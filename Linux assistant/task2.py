...
pyttsx3.speak("Hello boss")
print()
print("WELCOME TO MY INTELLIGENCE WORLD".center(125))                    
engine.runAndWait()
pyttsx3.speak("I am your voice assistant lara")
pyttsx3.speak("How can i help you...?")


while True:
  print()
  print("\t \t \t Tell me your requirement... listening : ", end='')
  ...
  if("hello" in ch) or ("hi" in ch) and ("heya" in ch) or ("hey" in ch):
    pyttsx3.speak("hello boss, how are you ...?")
    ...
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=rpm%20-q%20docker-ce")
    pyttsx3.speak("docker is not installed....")
  elif("configure docker" in ch) or ("install" in ch) and ("configure" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=yum%20install%20docker-ce%20--nobest%20-y")
    pyttsx3.speak("installing docker....")
  elif("start service" in ch) or ("start docker servie") and ("service") in ch:
    wb.open("http://1192.168.43.245/cgi-bin/local.py?x=systemctl%20start%20docker%20")
    ...
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=rpm%20-ivh%20 hadoop-1.2.1-1.x86_64.rpm%20--force")
    pyttsx3.speak("hadoop is successfully installed...")
  elif("create" in ch) or ("directory" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=mkdir%20/namenode")
    pyttsx3.speak("directory created")
  elif("setup or start" in ch) or ("namenode" in ch) or ("masternode" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=hadoop-daemon.sh%20start%20namenode")
    pyttsx3.speak("name node started")
  elif("report" in ch) or ("dfs" in ch) or ("admin" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=hadoop%20dfsadmin%20-report")
    pyttsx3.speak("here is the report of hadoop cluster")
  elif("SDK" in ch) or ("python library" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=pip3%20install%20boto3")
    pyttsx3.speak("boto three is already satisfied")
  elif("aws" in ch) or ("CLI" in ch) or ("command line" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=pip3%20install%20awscli")
    pyttsx3.speak("AWS CLI is already satisfied")
  elif("provision" in ch) or ("ec2" in ch) or ("instance" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=python3%20ec2.py")
    pyttsx3.speak("OS is successfully provisioned")
  elif("hping3" in ch) or ("establish" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=yum%20install%20hping3%20-y")
    pyttsx3.speak("H ping three is successfully installed")  
  elif("user" in ch) or ("newuser" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=useradd%20newuser")
    pyttsx3.speak("mickey user has been created")  
  elif("restrict" in ch) or ("boss" in ch):
    wb.open("http://192.168.43.245/cgi-bin/local.py?x=useradd%20-s%20/usr/bin/cal%20boss")
